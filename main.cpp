...
int main()
{
   ...
   // DynIbex Simulation
    ...
    //W[0] = Interval(40.0,90.0);
    //W[1] = Interval(0.09,0.61);
    ...
    S[1] = Interval(0.05, 0.7);


    const int n = 2;
	Variable x(n);
    Interval Te(0.0, 30.0);
    Interval Ti(10.0,10.0);
    ...
    // Te fisrt result was got with fs = 2.5
    double ef = 3.5;
    ...
    */
    ...
    ostringstream sa,sb,sc,sd,sk,stau;
    sa << R[0].lb();
    sb << R[0].ub();
    sc << R[1].lb();
    sd << R[1].ub();
    ...
    string name = string("patterns, tau=") + stau.str() + "min"+ ", K=" + sk.str()  + ", T=[" + sa.str()   + "," 
     + sb.str()  +  "]," + "V=[" + sc.str() +"," + sd.str() + "]" + string(".txt");
    ...
    while(!list_W.empty())
    {
        ...
        cout << "taille list_W : " << list_W.size() << std::endl;        
        ...
        if (result.empty()) {
            ...
            /*
            zon = np.matrix("[50, 50.3125] ; [0.09, 0.31]"); 
            if is_in_zonotope(X,zon):
                return np.matrix('[0,-1,-1;0,1,-1;0,1,2]') 
            */
            for (; it != result.end(); it++)
            {
                ...
                list<vector<int>> it_pat = (it->second);
                while(!it_pat.empty())
                {
                    vector<int>::const_iterator it_in_pat = it_pat.front().begin();
                    for(;it_in_pat != it_pat.front().end(); it_in_pat++)
                    {
                        file << *it_in_pat << ",";
                    }
                    if(it_pat.front().size()<k)
                    {
                        for(int i=0; i<(k-it_pat.front().size());i++)
                        {
                            file << "-1" << ",";
                        }                        
                    }
